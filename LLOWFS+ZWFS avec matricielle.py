# -*- coding: utf-8 -*-
"""
Created on Wed Sep  4 14:43:38 2024
...
abe_fourier_x=[]
abe_fourier_y=[]
for cycles in range(0, cycles_max + 1, 1):  # Incrémente de 50 cycles à chaque étape
...
I_ref_temp=(np.abs(E_ref_zwfs)**2)[[O==1]]#.flatten()
...
#sin_F1_temp=np.array([abe_fourier])*amp_calib[0]
#sin_F1=sin_F1_temp.reshape(len(abe_fourier), D,D)

#sin_F2_temp=np.array([abe_fourier])*amp_calib[1]
#sin_F2=sin_F2_temp.reshape(len(abe_fourier), D,D)
        
#abe_fourier_zwfs=np.concatenate([sin_F1, sin_F2], axis=0)


#%%
for q, zern in enumerate (OPD_calib):#abe_fourier_zwfs):
    
    E_A_zwfs_temp=1*O*np.exp((1j*2*np.pi*OPD_calib[q])/wavelength) # POLYNOMES DE ZERNIKE
    ...
    OPD_mesure_bis=OPD_mesure[[O==1]]#.flatten()
...
for i, cube in enumerate(zwfs_amp):
    ...
    OPD_mesure1=OPD_mesure_1bis[[O==1]]#.flatten()#[O==1]
    step_zwfs=((control_matrix_zwfs@((OPD_mesure1-I_ref_temp))))
...
#np.save('test_llowfs+zwfs_5DLIMS_LS_sbaru.txt',etude_mode)
#plt.xlim(-2E-7, 2E-7)   
#plt.ylim(-2E-7, 2E-7)      
...
a1=np.load('test_llowfs+zwfs_2DLIMS_LS_circular.txt.npy')
a=np.load('test_llowfs+zwfs_2DLIMS_LS_sbaru.txt.npy')
b=np.load('test_llowfs+zwfs_DLIMS_LS_circular.txt.npy')
c=np.load('test_llowfs+zwfs_DLIMS_LS_sbaru.txt.npy')
d=np.load('test_llowfs+zwfs_4DLIMS_LS_circular.txt.npy')
e=np.load('test_llowfs+zwfs_4DLIMS_LS_sbaru.txt.npy')
#f=np.load('test_llowfs+zwfs_5DLIMS_LS_circular.txt.npy')
g=np.load('test_llowfs+zwfs_5DLIMS_LS_sbaru.txt.npy')
#h=np.load('test_llowfs+zwfs_5DLIMS_LS_circular.txt.npy')
#i=np.load('test_llowfs+zwfs_5DLIMS_LS_sbaru.txt.npy')


plt.plot(cube_amp_rad,a1,"--",label='D_lim_LS = 2 x D CIRCULAR')
plt.plot(cube_amp_rad,a,"--",label='D_lim_LS = 2 x D SUBARU ')
plt.plot(cube_amp_rad,b,"--",label='D_lim_LS = 3 x D CIRCULAR ')
plt.plot(cube_amp_rad,c,"--",label='D_lim_LS = 3 x D SUBARU' )
plt.plot(cube_amp_rad,d,"--",label='D_lim_LS = 4 x D CIRCULAR')
plt.plot(cube_amp_rad,e,"--",label='D_lim_LS = 4 x D SUBARU')
#plt.plot(cube_amp_rad,f,"--",label='D_lim_LS = 5 x D CIRCULAR')
plt.plot(cube_amp_rad,g,"--",label='D_lim_LS = 5 x D SUBARU')
plt.xlabel('error applied (nm)')
plt.ylabel('error measured (nm)')
    
plt.title(' ZWFS + LLOWFS response  du defocus FQPM  mask = 1.06 \u03BB /D')
plt.legend()
"""
"""
...
plt.xlabel('error applied (nm)')
plt.ylabel('error measured (nm)')
plt.title(' ZWFS + LLOWFS response VORTEX lp=8 SUBARU  mask = 1.06 \u03BB /D')
plt.legend()
"""

#%%CHARGER DONNEES
chemin = Path(r"C:\Users\arahim\Nextcloud\PhD\simu\resultats LLOWFS+ZWFS\response curves\sensor ZWFS sans defoc\FQPM SUBARU")
"""
cube_amp_rad=zwfs_amp
dernier_element=chemin.name
a1 = np.load(chemin / 'test_zwfs0.txt.npy')
a = np.load(chemin / 'test_zwfs1.txt.npy')
b = np.load(chemin / 'test_zwfs2.txt.npy')
c = np.load(chemin / 'test_zwfs3.txt.npy')
d = np.load(chemin / 'test_zwfs4.txt.npy')
e = np.load(chemin / 'test_zwfs5.txt.npy')
f = np.load(chemin / 'test_zwfs6.txt.npy')
g = np.load(chemin / 'test_zwfs7.txt.npy')
h = np.load(chemin / 'test_zwfs8.txt.npy')
i = np.load(chemin / 'test_zwfs9.txt.npy')
...
plt.xlim(-2E-7, 2E-7)   
plt.ylim(-2E-7, 2E-7)  
plt.title(f' ZWFS + LLOWFS response {dernier_element}  mask = 1.06 \u03BB /D')
plt.legend()
"""