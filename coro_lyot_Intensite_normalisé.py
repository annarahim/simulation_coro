# -*- coding: utf-8 -*-
"""
Created on Mon Nov 27 15:12:21 2023
...
E_B=np.fft.fftshift(np.fft.fft2(np.fft.fftshift(E_A),norm='ortho'))
I_B=np.abs(E_B)**2

plt.figure('plan B avant MFP')
...
#création du MFP
MFP=(grid_size/circle_radius)*1.2
non_radius = (x-x0)**2 + (y-y0)**2 <= MFP**2
P_B=np.ones((grid_size,grid_size))
P_B[non_radius]=0
plt.figure('MFP')
plt.imshow(P_B)
plt.colorbar(label='Intensité')
...
plt.imshow(I_B2**0.25)
plt.colorbar(label='Intensité')

E_C=np.fft.fftshift(np.fft.ifft2(np.fft.fftshift(E_B2),norm='ortho'))
I_C=np.abs(E_C)**2
...
#creation du LS
MFP_inver=(MFP*0.7)
radius2 = (x-x0)**2 + (y-y0)**2 <= (MFP_inver)**2
...
plt.plot(np.log10(I_Bnorm[grid_size//2,grid_size//2:grid_size]), color='blue', alpha=1,label='plan D')
plt.plot(np.log10(I_Dnorm[grid_size//2,grid_size//2:grid_size]), color='red', alpha=1,label='plan B')
plt.legend()
